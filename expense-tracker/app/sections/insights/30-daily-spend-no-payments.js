...
// Build a Set of "major|minor" keys for categories where is_subscription_eligible = true.
// Transactions in those categories are considered "payments" and excluded.
function _paymentCategoryKeys() {
  const keys = new Set();
  state.categories.forEach(c => {
    const flag = c.is_subscription_eligible;
    if (flag === true || flag === 'TRUE' || flag === 'true') {
      keys.add(`${c.major_category}|${c.minor_category}`);
    }
  });
  return keys;
}
...
function _buildData(txs, from, to) {
  const paymentKeys = _paymentCategoryKeys();
  const filtered    = txs.filter(t =>
    t.tx_type === 'money-out' &&
    !paymentKeys.has(`${t.major_category}|${t.minor_category}`)
  );
...
export async function render(containerId, { txs, from, to, sym }) {
  ...
  if (!container) {
    console.warn('[insight-30] container not found:', containerId);
    return null;
  }

  const { labels, values, dayTxs } = _buildData(txs, from, to);
  ...
  const chart = new Chart(canvas, {
    ...
    data: {
      labels,
      datasets: [{
        label: 'Daily spend (excl. payments)',
        data:  values,
  ...
  console.log(`[insight-30] daily spend (no payments) — ${labels.length} days, total=${values.reduce((s, v) => s + v, 0).toFixed(0)}`);
  return chart;
}
