...
Schema reference: subscription-schema.gs, subscription-core.gs, subscription-validation.gs, subscription-utils.gs.
...
|---|---|---|---|---|
| 1 | `id` | string | No | System-generated UUID. Generated by `Utilities.getUuid()`. Globally unique; no date prefix, no counter. |
| 2 | `subscription_name` | string | Yes | User-facing display name for the subscription. Must be unique across non-deleted subscriptions (case-insensitive, trimmed). |
...
| 12 | `description` | string | Yes | Free-text notes. Optional. UI label: "Notes". |
| 13 | `record_status` | enum | Yes | Lifecycle state. Valid values: `active`, `inactive`, `deleted`, `locked`. Default on creation: `active` (or `inactive` if `subscription_end_date_local` is already in the past). Updatable via `update_subscription` to `active` or `inactive` only (`invalid_record_status` is returned for any other value). |
| 14 | `created_at` | string | No | ISO 8601 UTC timestamp set at creation. |
| 15 | `sync_status` | string | No | Sync pipeline state. Valid values: `create-pending`, `update-pending`, `in-sync`, `create-failed`, `update-failed`. Set to `create-pending` on creation; advanced by `computeSyncStatus()` on each mutation. |
| 16 | `sync_date` | string | No | Timestamp of the last successful sync. Written by the sync pipeline. |
| 17 | `sync_notes` | string | No | Notes written by the sync pipeline (e.g. error detail). Cleared on each mutation. |
| 18 | `updated_at` | string | No | ISO 8601 UTC timestamp of the last write. Stamped on both create and update. |
| 19 | `subscription_start_date_local` | string | Yes | ISO datetime (`YYYY-MM-DD HH:MM:SS`) from which the subscription is active. Optional; informational only — does not affect `next_payment_date` computation or `record_status`. |
| 20 | `subscription_end_date_local` | string | Yes | ISO datetime (`YYYY-MM-DD HH:MM:SS`) after which the subscription expires. Optional. When present and past (date portion compared), triggers lazy auto-expiry (see below). |
| 21 | `subscription_timezone_local` | string | Yes | IANA timezone identifier (e.g. `Europe/London`, `America/New_York`). Optional; stored as-is. Informational — `computeNextPaymentDate` uses server local time regardless of this value. |
...
A subscription created with a `subscription_end_date_local` already in the past is written with `record_status = inactive` immediately at creation (the initial status check runs in `createSubscription` before the row is appended).
