...
3. **Amount consistency check:** compute mean and stdDev of `toBase(tx.tx_amount_local, account.local_currency)` per group. Skip if `stdDev / mean > 0.05` (more than 5% variance → not a fixed recurring charge).
