...
def fetch_latest() -> dict[str, float]:
    ...
    resp = requests.get(f"{BASE_URL}/latest", params={"from": "USD", "to": CURRENCIES}, timeout=30)
...
def fetch_date(rate_date: date) -> dict[str, float]:
    ...
    resp = requests.get(f"{BASE_URL}/{rate_date.isoformat()}", params={"from": "USD", "to": CURRENCIES}, timeout=30)
    resp.raise_for_status()
    return resp.json()["rates"]
