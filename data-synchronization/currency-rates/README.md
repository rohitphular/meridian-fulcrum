...
**Crypto (3)** — sourced from exchangerate.fun (no rank, fetched in a single call):
...
1. **Never-fetched first** (`data_last_fetched IS NULL`) — currencies with no data at all hit the API before the limit kicks in
2. **Then by `currency_rank` ASC** — within currencies that have been fetched, higher-ranked ones go before lower-ranked ones

SQL: `ORDER BY data_last_fetched ASC NULLS FIRST, currency_rank ASC NULLS LAST`

After each successful fetch, `data_last_fetched` is updated to the latest date returned for that currency. If a run is cut short by a rate limit, the next run automatically starts with the currencies that were missed.
...
- A random 1–5 second delay is added between each currency request to avoid rate limiting
...
| Column              | Type        | Notes |
|---------------------|-------------|-------|
| `currency_code`     | CHAR(3)     | Primary key (e.g. `USD`, `XAU`) |
...
| `data_last_fetched` | DATE        | Latest date for which we have rate data. NULL = never fetched |
...
```yaml
job:
  start_date: "2020-01-01"   # used as the floor date in historical load
...
| `CR_DB_PORT`           | Postgres port (default `5432`) |
| `CR_DB_USER`           | Postgres user |
| `CR_DB_PASSWORD`       | Postgres password |
| `CR_DB_NAME`           | Postgres database name |
| `CR_HISTORICAL_CSV_DIR`| Absolute path to the folder containing downloaded stooq CSV files |
...
```
currency-rates/
├── config.yaml              # source toggles and start date
...
├── database/
│   ├── currency_master.py   # fetch-order query and data_last_fetched updates
