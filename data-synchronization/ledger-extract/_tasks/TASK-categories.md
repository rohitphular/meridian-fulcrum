...
## Sheet schema (21 columns)

| # | Column | Notes |
|---|--------|-------|
| 1 | `id` | UUID — stamped by GAS on create; never changed; authoritative row identifier |
| 2 | `tx_type_key` | Natural key part 1 — `money-in` or `money-out` |
| 3 | `tx_type_label` | Display label for tx type |
| 4 | `major_category_key` | Natural key part 2 — slug derived from label |
| 5 | `major_category_label` | User-facing label |
| 6 | `minor_category_key` | Natural key part 3 — slug derived from label |
| 7 | `minor_category_label` | User-facing label |
| 8 | `description` | Optional |
| 9 | `tag_keywords` | Comma-and-space-separated; lowercased on save |
| 10 | `counterparty_examples` | Optional |
| 11 | `source_account_types` | Comma-separated sub-types |
| 12 | `target_account_types` | Comma-separated sub-types |
| 13 | `source_account_mandatory` | `TRUE` / `FALSE` |
| 14 | `target_account_mandatory` | `TRUE` / `FALSE` |
| 15 | `is_subscription_eligible` | `TRUE` / `FALSE` |
| 16 | `record_status` | `active`, `inactive`, `deleted`, `locked` |
| 17 | `sync_status` | Backend-stamped — extractor writes back cols 17–19 only |
| 18 | `sync_date` | Backend-stamped |
| 19 | `sync_notes` | Backend-stamped |
| 20 | `created_at` | Backend-stamped — never written by extractor |
| 21 | `updated_at` | Backend-stamped — never written by extractor |

Columns 1–16 are source data read by the extractor. Columns 17–21 are stamped by the GAS backend; the extractor writes back only cols 17–19 via the designated write-back path and must never touch cols 20–21.
...
|--------|-----------|---------|-------|
| `id` | 1 | `UUID NOT NULL` | PK — read from sheet col 1; stamped by GAS on create; never regenerated by the extractor |
| `tx_type_key` | 2 | `TEXT NOT NULL` | Natural key part 1 — transform hard errors if not `money-in` or `money-out` |
| `tx_type_label` | 3 | `TEXT NOT NULL` | Transform hard errors if empty |
| `major_category_key` | 4 | `TEXT NOT NULL` | Natural key part 2 — transform hard errors if empty or contains `|` |
| `major_category_label` | 5 | `TEXT NOT NULL` | Transform hard errors if empty |
| `minor_category_key` | 6 | `TEXT NOT NULL` | Natural key part 3 — transform hard errors if empty or contains `|` |
| `minor_category_label` | 7 | `TEXT NOT NULL` | Transform hard errors if empty |
| `description` | 8 | `TEXT` | |
| `tag_keywords` | 9 | `TEXT` | |
| `counterparty_examples` | 10 | `TEXT` | |
| `source_account_mandatory` | 13 | `BOOLEAN NOT NULL` | Default `FALSE` if empty or None |
| `target_account_mandatory` | 14 | `BOOLEAN NOT NULL` | Default `FALSE` if empty or None |
| `is_subscription_eligible` | 15 | `BOOLEAN NOT NULL DEFAULT FALSE` | Default `FALSE` if empty or None |
| `record_status` | 16 | `TEXT NOT NULL` | Transform hard errors if empty, None, or not in `{'active', 'inactive', 'deleted', 'locked'}`; mirrored verbatim from sheet |
...
- `sync_status` (col 17): `'in-sync'` on success; `'create-failed'` or `'update-failed'` (matching the path taken) on DB error or validation failure
- `sync_date` (col 18): UTC ISO timestamp of the operation
- `sync_notes` (col 19): `''` on success; error message on failure
...
0. Read `sync_status` (col 17): if `'in-sync'`, skip immediately — no further processing, no sheet write-back
...
4. On success: write `sync_status = 'in-sync'`, `sync_date = <UTC ISO timestamp>`, `sync_notes = ''` to the sheet for this row.
