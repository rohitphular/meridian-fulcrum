...
**Natural key encoding in `extract_hashes`:** `{tx_type}|{major_category}|{minor_category}`
...
4. For each key present in DB but absent from sheet — **within a single transaction**:
   ...
   - Resolve to `category_id`: `UPDATE category_master SET is_deleted = TRUE, deleted_at = now(), updated_at = now() WHERE tx_type = $1 AND major_category = $2 AND minor_category = $3 RETURNING id`; if no row returned (DB inconsistency), log error, rollback the transaction, and skip — leave the `extract_hashes` row intact. This is a permanently stuck state: every subsequent run will hit the same missing `category_master` row and skip again. Manual intervention is required: investigate the root cause, then `DELETE FROM extract_hashes WHERE entity = 'categories' AND natural_key = '<key>'`
   ...
   - `DELETE FROM extract_hashes WHERE entity = 'categories' AND natural_key = $natural_key`
...
- [x] `database/upsert.py` — category_master upsert + explicit join table deletes/inserts
- [x] `database/hashes.py` — read/write extract_hashes for categories
- [x] `database/job_state.py` — Phase 1 bootstrap/read and Phase 3 UPSERT
