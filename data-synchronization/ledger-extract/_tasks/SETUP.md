...
├── transforms/
│   ├── categories.py
│   ├── accounts.py          # sheet row → typed dict + hash (not yet built)
│   ├── transactions.py      # (not yet built)
│   └── subscriptions.py     # (not yet built)
├── database/
│   ├── models/              # generated by make generate-models
│   ├── hashes.py            # read/write extract_hashes table
│   ├── job_state.py         # Phase 1 bootstrap/read and Phase 3 UPSERT for job_state
│   └── upsert.py            # entity upsert helpers
└── migrations/
...
exclude           = ["schema_migrations"]   # py-db-migrate tracking table — not a data model
```
...
## `extract_hashes` table schema (migration 0007)
...
Rows are deleted from this table when the corresponding entity row is soft-deleted — `extract_hashes` only holds currently active keys.

---

## `job_state` table schema (migration 0008)
...
1. Bootstrap: `INSERT INTO job_state (job_name, last_sheet_modified_at, ran_at) VALUES ('ledger-extract', NULL, '1970-01-01T00:00:00Z'::timestamptz) ON CONFLICT (job_name) DO NOTHING` — ensures a row exists on first run; sentinel `ran_at` is immediately overwritten by Phase 3 on success
2. Read `last_sheet_modified_at` from `job_state WHERE job_name = 'ledger-extract'`
3. Call `SheetsClient.get_modified_time()` and cache the result — it is reused in Phase 3. If an error occurs (API or network error), `get_modified_time()` raises — the exception propagates and aborts the job run; do not proceed, do not update `ran_at`. If the cached result equals `last_sheet_modified_at`, run `UPDATE job_state SET ran_at = now() WHERE job_name = 'ledger-extract'` and exit (intentional — `last_sheet_modified_at` already equals the current sheet modified time; the next run will compare against the same value and early-exit again if the sheet is still unchanged). Note: on first run `last_sheet_modified_at IS NULL` — NULL never equals a real timestamp, so the job always proceeds on first run.
...
4. For each enabled entity (categories → accounts → transactions → subscriptions):
   ...
   c. Compare against `(entity, natural_key, row_hash)` stored in `extract_hashes`
   d. Upsert rows where hash changed or key is new — **each row's writes within a single DB transaction**
   e. After all rows for this entity processed, query `extract_hashes` for keys not seen this run — soft-delete those rows — **each deletion within a single DB transaction**

**Phase 3 — Finalise**
5. Update job_state using the `get_modified_time()` value cached in Phase 1:
...
1. `categories` — includes `account_types` (0001) + category_master (0002) + join tables (0003) + **shared infrastructure: `extract_hashes` (0007) and `job_state` (0008) are built here as part of the first entity**
