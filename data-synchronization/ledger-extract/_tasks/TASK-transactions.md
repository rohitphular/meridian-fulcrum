...
| 2 | `tx_date_local` | `YYYY-MM-DD HH:MM:SS` string | No | Required — stores LOCAL time (not UTC); converted to UTC by the extract using `tx_timezone_local`; maps to `tx_date_time_base` |
| 3 | `tx_timezone_local` | string | No | Optional — IANA timezone name (e.g. `Asia/Kolkata`); defaults to `Europe/London` when blank; maps to `tx_timezone_local` |
...
| 6 | `account_id` | string | No | Required — UUID (PK of `account_master`; stamped by GAS on account creation); looked up directly in the preloaded `account_map`; replaces the old `source_account` + `target_account` columns; currency is derived from the resolved account's `local_currency` — there is no separate `currency` column in the sheet |
| 7 | `tx_amount_local` | number string | No | Required — amount in the account's local currency major units; maps to `tx_amount_local` |
...
| 13 | `beneficiaries` | string | No | Optional — semicolon-separated names with optional percentages (e.g. `Alice:60;Bob:40`); drives `beneficiaries_master` + `transaction_beneficiaries`; blank sets `beneficiaries_raw = None` and skips beneficiary resolution entirely — no error |
...
| 20 | `sync_status` | string | Yes | `create-pending`, `update-pending`, `in-sync`, `create-failed`, `update-failed`; extract writes result after processing |
| 21 | `sync_date` | ISO datetime string | Yes | Written by the extract on every sync attempt — separate column from `sync_notes`, mirroring the accounts module pattern |
| 22 | `sync_notes` | string | Yes | Failure reason written by the extract on `create-failed`/`update-failed`; cleared on success |
...
| `tx_date_time_base` | `TIMESTAMPTZ NOT NULL` | | `tx_date_local` | Sheet stores LOCAL time — extract converts to UTC using `tx_timezone_local` before storing |
...
| `tx_timezone_local` | `TEXT NOT NULL` | | `tx_timezone_local` | IANA timezone name; `Europe/London` when sheet column blank |
...
| `category_id` | `UUID NOT NULL` | FK → `category_master(id)` | `tx_type` + `major_category` + `minor_category` | NOT NULL — `create-failed`/`update-failed` if no match; direction (money-in vs money-out) encoded in the matched category's own `tx_type` field; transfer rows are distinguished solely by linked `parent_tx_id`, not by a separate `tx_type` column |
...
| `tx_amount_local` | `BIGINT NOT NULL` | `CHECK (tx_amount_local > 0)` | `tx_amount_local` | Local currency minor units (e.g. pence for GBP, nanograms for XAU) |
...
- `account_id → (account_master.id, local_currency)` map (accounts with `record_status NOT IN ('deleted', 'locked')` only):
  ```sql
  SELECT id, local_currency FROM account_master
  WHERE record_status NOT IN ('deleted', 'locked')
  ```
  `account_map: dict[str, tuple[UUID, str]]` — maps UUID string → (UUID, local_currency). Used to resolve `account_id` (UUID from sheet) and derive `local_currency` in one lookup.
...
**Write-back on success:** `sync_status = 'in-sync'`, `sync_date = <now ISO>`, `sync_notes = ''`, `created_at = <original or now ISO>`, `updated_at = <now ISO>` (5 columns starting at `sync_status` col position)

**Write-back on failure:** `sync_status = 'create-failed'` or `'update-failed'` (matching the row's status group), `sync_date = <now ISO>`, `sync_notes = <reason string>` (3 columns only — `created_at` / `updated_at` are not written on failure)
...
- `tx_type` required; must be `money-in` or `money-out` → on failure: `create-failed`/`update-failed`
- `account_id` required; must exist in the preloaded `account_map` → on failure: `create-failed`/`update-failed: account_not_found`; `local_currency` is taken from the matched account's `local_currency`
- `tx_amount_local` required; must be a valid positive decimal → on failure: `create-failed`/`update-failed`
- `record_status` required; must be `active`, `inactive`, `deleted`, or `locked` → on failure: `create-failed`/`update-failed`
- `beneficiaries` optional; if blank → `beneficiaries_raw = None`; beneficiary resolution is skipped entirely

**Datetime computation:**
- `tx_timezone_local` = `raw_tx_timezone_local.strip()` if non-blank, else `'Europe/London'`
- Validate `tx_timezone_local` via `ZoneInfo(tx_timezone_local)` — unrecognised IANA name → `create-failed`/`update-failed`; blank is always safe (defaults to `'Europe/London'` before validation)
- `tx_timezone_base` = `'UTC'` — always
- `tx_date_time_base` = `datetime.fromisoformat(raw_tx_date_local.strip()).replace(tzinfo=ZoneInfo(tx_timezone_local)).astimezone(ZoneInfo(tx_timezone_base))` — `fromisoformat` returns a naive datetime; `.replace(tzinfo=ZoneInfo(tx_timezone_local))` stamps the local timezone without shifting; `.astimezone(ZoneInfo("UTC"))` then converts to UTC
...
9. Commit
10. Write `in-sync` to `sync_status`, set `sync_date`, clear `sync_notes`, write `created_at` / `updated_at`

On any failure — whether a raised exception or a controlled failure (e.g. `account_not_found` at step 2, `parent_tx_not_found` at step 3, or a beneficiary validation failure at step 8) — call `conn.rollback()` before writing a failure status to the sheet and moving to the next row. All failures write `create-failed` so they are auto-retried on the next run once the missing data is added. Steps 5 onwards write to the open transaction; without an explicit rollback those writes bleed into the next row's commit. **Exception — UNIQUE violation on `transaction_id`:** if the INSERT fails with a unique constraint violation, do not write `create-failed`; instead fall through to the `update-pending` path (fetch original `created_at`, delete + re-insert). This handles the case where a previous run committed successfully but the sheet write-back failed, leaving the row stuck on `create-pending`. Implementation note: in psycopg2, after any constraint violation the transaction is in an aborted state — no further SQL can run until a rollback or savepoint rollback. The INSERT must be wrapped in a `SAVEPOINT`; on `UniqueViolation` rollback to the savepoint (not the full transaction) before executing the update-pending SELECT + delete + re-insert.
...
1. Look up `transaction_master.id` (surrogate PK), `record_status`, and `created_at` WHERE `transaction_id = sheet_id`
   - Not found → `update-failed: transaction_not_found`; skip
   - `record_status = 'locked'` → `update-failed: transaction_locked`; skip
   - `record_status = 'deleted'` → `update-failed: transaction_deleted`; skip
