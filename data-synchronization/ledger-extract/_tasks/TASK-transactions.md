...
| Q8 | `tx_amount` sign convention | `tx_amount_local > 0` enforced by CHECK — sign is conveyed by `tx_type`. |
...
| 4 | `parent_tx_id` | string | No | Optional — references another transaction's `id`; DB mapping TBD (see open design questions below) |
| 5 | `tx_type` | enum string | No | `money-in`, `money-out`, `money-transfer` |
...
| 19 | `record_status` | enum string | No | Controls lifecycle — values and DB mapping TBD (see open design questions below); replaces the concept previously captured in `tx_status` DB column (Q16) |
...
| `tx_type` | `TEXT NOT NULL` | `CHECK (tx_type IN ('money-in', 'money-out'))` | `tx_type` | `money-transfer` is not a value — a transfer is represented as two linked rows (`money-out` on the source account, `money-in` on the target), connected via `parent_tx_id` |
...
chk_tm_tx_type           CHECK (tx_type IN ('money-in', 'money-out'))
...
6. Resolve `category_id`: look up `(tx_type, major_category, minor_category)` in `category_master`; NULL if no match; log warning
...
| `CheckViolation` on `chk_tm_tx_type` | `"Invalid tx_type — must be money-in or money-out"` |
