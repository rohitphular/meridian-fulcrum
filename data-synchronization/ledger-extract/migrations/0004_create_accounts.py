    ...
    with client.cursor() as cursor:
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS account_master (
                ...
                opening_value       NUMERIC(19,6) NOT NULL,
                ...
                CONSTRAINT chk_account_master_account_type      CHECK (account_type IN ('asset', 'investment', 'liability')),
                CONSTRAINT chk_account_master_record_status     CHECK (record_status IN ('active', 'inactive', 'deleted', 'locked')),
                CONSTRAINT chk_account_master_opening_value_sign CHECK (
                    (account_type IN ('asset', 'investment') AND opening_value >= 0) OR
                    (account_type = 'liability' AND opening_value <= 0)
                )
            );
        """)
